...
public class ProductServiceTests
{
    ...
    private void SetupQuery(List<Product> products)
    {
        var mockDbSet = MoqExtensions.BuildMockDbSet(products);
        _productRepositoryMock.Setup(r => r.Query()).Returns(mockDbSet.Object);
    }
    ...
    public async Task CreateAsync_SuccessPath_AddsProductAndSaves()
    {
        ...
        var category = new Category("Furniture", "Office and Gaming Furniture");
    ...
    public async Task UpdateAsync_SuccessPath_FullUpdate_TriggersChangeMethodsAndSaves()
    {
        ...
        var product = new Product(oldCategoryId, "Old Name", "Old Desc", 100m, 5, "old.png");
        var request = new UpdateProductRequest(newCategoryId, "New Name", "New Desc", 150m, 10, "new.png");
        var validCategory = new Category("New Category", "Category Desc");
    ...
    [Fact]
    public async Task UpdateAsync_SuccessPath_SameData_StillExecutesAndSaves()
    {
        var productId = Guid.NewGuid();
        var categoryId = Guid.NewGuid();
    ...
    public async Task UpdateAsync_FailCategoryNotFound_ThrowsKeyNotFoundException()
    {
        ...
        var oldCategoryId = Guid.NewGuid();
        var newCategoryId = Guid.NewGuid();
        var product = new Product(oldCategoryId, "Name", "Desc", 100m, 5, "url.png");
    ...
    public async Task UpdateAsync_FailDuplicateName_ThrowsInvalidOperationException()
    {
        var productId = Guid.NewGuid();
        var categoryId = Guid.NewGuid();
        var product = new Product(categoryId, "Old Name", "Desc", 100m, 5, "url.png");
        var request = new UpdateProductRequest(null, "Existing Name", null, null, null, null);

        _productRepositoryMock.Setup(r => r.GetByIdAsync(productId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(product);

        var duplicateProduct = new Product(categoryId, "Existing Name", "Desc", 120m, 10, "url2.png");

        SetupQuery(new List<Product> { duplicateProduct });
    ...
    {
        var productId = Guid.NewGuid();
        var product = new Product(Guid.NewGuid(), "Name", "Desc", 10m, 5, null);
    ...
    public async Task GetAllAsync_SuccessPath_ReturnsMappedCollection()
    {
        ...
        {
            new Product(Guid.NewGuid(), "P1", "D1", 10m, 2, null),
            new Product(Guid.NewGuid(), "P2", "D2", 20m, 4, null)
        };
    ...
    public async Task GetPagedAsync_SuccessPath_ReturnsCorrectProductPagedResponse()
    {
        ...
        var products = new List<Product> { new Product(Guid.NewGuid(), "Chair", "Comfortable", 150m, 5, null) };
